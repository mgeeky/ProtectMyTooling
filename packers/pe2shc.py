...
class PackerPe2shc(IPacker):
    ...
    @staticmethod
    def get_desc():
        return 'pe_to_shellcode by Hasherezade, takes PE EXE/DLL and produces PIC shellcode'
    ...
    def help(self, parser):
        if parser != None:
            ...
            self.options['pe2shc_path'] = os.path.abspath(configPath(self.options['config'], self.options['pe2shc_path']))
    ...
    def process(self, arch, infile, outfile):
        ...
        if not status:
            ...
            if len(out) > 0 and not (self.options['verbose'] or self.options['debug']): self.logger.info(f'''{PackerPe2shc.get_name()} returned:
----------------------------------------
{out}
----------------------------------------
