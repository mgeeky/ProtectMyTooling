...
class PackerAsStrongAsFuck(IPacker):
    ...
    metadata = {
        'author': ['Charterino', 'klezVirus'],
        'url': 'https://github.com/Charterino/AsStrongAsFuck',
        'description': 'Console obfuscator for .NET assemblies',
    ...
    def help(self, parser):
        if parser != None:
            ...
            if not os.path.isfile(self.options['asstrongasfuck_path']):
                self.logger.fatal(
                    '--asstrongasfuck-path option must be specified!')
            ...
            if 'asstrongasfuck_opts' in self.options.keys() and self.options['asstrongasfuck_opts'] != None \
                ...
                self.asstrongasfuck_args += ' -o ' + \
                    self.options['asstrongasfuck_opts']
    ...
    def process(self, arch, infile, outfile):
        ...
        if os.path.isfile(infile + '.obfuscated'):
            ...
            if len(out) > 0 and not (self.options['verbose'] or self.options['debug']):
                self.logger.info(f'''{PackerAsStrongAsFuck.get_name()} returned:
----------------------------------------
{out}
----------------------------------------
