...
class PackerSgn(IPacker):
    ...
    @staticmethod
    def get_type():
        return PackerType.ShellcodeEncoder

    @staticmethod
    def get_desc():
        return 'Shikata ga nai (仕方がない) encoder ported into go with several improvements. Takes shellcode, produces encoded shellcode.'
    ...
    def help(self, parser):
        if parser != None:
            ...
            parser.add_argument('--sgn-rounds', metavar='NUM', type=int, dest='sgn_rounds', help = 'Number of times to encode the binary (increases overall size) (default 1)')
            parser.add_argument('--sgn-max', metavar='NUM', type=int, dest='sgn_max', help = 'Maximum number of bytes for obfuscation (default 20)')
            ...
            if 'sgn_path' in self.options.keys() and self.options['sgn_path'] != None and len(self.options['sgn_path']) > 0:
                self.options['sgn_path'] = configPath(self.options['config'], self.options['sgn_path'])
            else:
    ...
    def process(self, arch, infile, outfile):
        try:
            if arch == 'x86': _arch = 32
            elif arch == 'x64': _arch = 64

            self.sgn_args = self.sgn_args.replace('<arch>', str(_arch))

            if self.options['debug']: self.sgn_args += ' -v'

            self.sgn_args = self.sgn_args.replace('<max>', str(self.options['sgn_max']))
            self.sgn_args = self.sgn_args.replace('<rounds>', str(self.options['sgn_rounds']))
            ...
            if os.path.isfile(outfile):
                ...
                if len(out) > 0: self.logger.info(f'''Sgn returned:
----------------------------------------
{out}
----------------------------------------
