...
class PackerThemida(IPacker):
    ...
    metadata = {
        ...
        'description': 'Advanced x86/x64 PE Executables virtualizer, compressor, protector and binder',
    ...
    def help(self, parser):
        if parser != None:
            ...
            if not os.path.isfile(self.options['themida_path_x86']) or not os.path.isfile(self.options['themida_path_x64']):
                self.logger.err(
                    'Both options --themida-path-x86 and --themida-path-x64 must be specified!')

            if not os.path.isfile(self.options['themida_project_file']):
                self.logger.fatal(
                    'You must specify existing --themida-project-file project file (.tmd) that will be reused for subsequent samples!')
    ...
    def process(self, arch, infile, outfile):
        ...
        if not path:
            self.logger.fatal(
                'Architecture {} not supported as there was no corresponding Themida Protector path configured!\nPlease configure it using: --themida-path-{}'.format(arch, arch))
        ...
        try:
            ...
            self.logger.info(
                "Running Themida protector, heavy computations ahead, be patient...")
        ...
        if not status:
            ...
            if len(out) > 0 and not (self.options['verbose'] or self.options['debug']):
                self.logger.info(f'''{PackerThemida.get_name()} returned:
----------------------------------------
{out}
----------------------------------------
