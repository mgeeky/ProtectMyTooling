...
class PackerScareCrow(IPacker):
    ...
    def help(self, parser):
        if parser != None:
            ...
            parser.add_argument('--scarecrow-password', metavar='PASS', dest='scarecrow_password', help = 'The password for code signing cert. Required when -valid is used')
            parser.add_argument('--scarecrow-inject', metavar='PROCESS', dest='scarecrow_inject', help = 'Enables Process Injection Mode and specify the path to the process to create/inject into (use \\ for the path)')
            parser.add_argument('--scarecrow-sandbox', action='store_true', dest='scarecrow_sandbox', help = 'Enables sandbox evasion using IsDomainJoined calls.')
            ...
            if 'scarecrow_path' in self.options.keys() and self.options['scarecrow_path'] != None and len(self.options['scarecrow_path']) > 0:
                self.options['scarecrow_path'] = configPath(self.options['config'], self.options['scarecrow_path'])
            else:
    ...
    def process(self, arch, infile, outfile):
        try:
            ...
            if outfile.lower().endswith('.dll'): loader = 'dll'
            elif outfile.lower().endswith('.exe'): loader = 'binary'
            ...
            if len(self.options['scarecrow_loader']) > 0: 
                ...
                if not outfile.lower().endswith('.js') and not outfile.lower().endswith('.hta'):
                    self.logger.fatal('When using ScareCrow -Loader different than binary/dll output file must have .js/.hta extension!')

            if self.options['scarecrow_sandbox']: self.scarecrow_args += ' -sandbox'
            if len(self.options['scarecrow_inject']) > 0: self.scarecrow_args += f' -injection "{self.options["scarecrow_inject"]}"'

            if len(self.options['scarecrow_valid']) > 0:
                self.scarecrow_args += ' -valid ' + self.options['scarecrow_valid']
                ...
                self.scarecrow_args += ' -domain ' + self.options['scarecrow_domain']
                self.options['scarecrow_sign'] = True

            if not self.options['scarecrow_sign']: self.scarecrow_args += ' -nosign'
            ...
            if os.path.isfile(outfile):
                ...
                if len(out) > 0 and not (self.options['verbose'] or self.options['debug']): self.logger.info(f'''{PackerScareCrow.get_name()} returned:
----------------------------------------
{out}
----------------------------------------
