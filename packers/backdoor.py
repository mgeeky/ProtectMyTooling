...
#
# Name:   jadams/Backdoor 
# URL:    https://github.com/jadams/Backdoor
...
class PackerBackdoor(IPacker):
    ...
    @staticmethod
    def get_desc():
        return 'RedBackdoorer - backdoors legitimate PE executable with specified shellcode'
    ...
    def help(self, parser):
        if parser != None:
            ...
            if 'backdoor_path' in self.options.keys() and self.options['backdoor_path'] == '':
                ...
                self.options['backdoor_path'] = os.path.abspath(configPath(self.options['config'], self.options['backdoor_path']))

            self.options['backdoor_file'] = os.path.abspath(self.options['backdoor_file'])

            if not dontCheckExists and not os.path.isfile(self.options['backdoor_path']):
                self.logger.fatal('Missing --backdoor-path option! It must point to RedBackdoorer.py script!')

            if not os.path.isfile(self.options['backdoor_file']):
                self.logger.fatal('Missing -B / --backdoor-file option! It must point to a legitimate PE file that you wish to backdoor.')
            ...
            if len(self.options['custom_ioc']) > 0:
                self.backdoor_args += ' --ioc "' + self.options['custom_ioc'] + '"'
    ...
    def process(self, arch, infile, outfile):
        ...
        if not status:
            ...
            if len(out) > 0 and not (self.options['verbose'] or self.options['debug']): self.logger.info(f'''{PackerBackdoor.get_name()} returned:
----------------------------------------
{out}
----------------------------------------
